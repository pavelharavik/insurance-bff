package com.insurance.bff.domain.insurance;

/**
 * Thrown by the service layer when insurance data could not be retrieved from any upstream. Carries
 * a human-readable message and the {@link UpstreamErrorType} for HTTP status mapping by the
 * presentation layer.
 */
public class InsuranceDataUnavailableException extends RuntimeException {
  ...
  /**
   * @param detail additional context extracted from the upstream error response, appended to the
   *               base message
   */
