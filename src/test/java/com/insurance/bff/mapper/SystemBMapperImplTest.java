...
/**
 * Unit tests for {@link SystemBMapperImpl}.
 * Validates XML attribute mapping, name joining, and active flag in isolation.
 */
class SystemBMapperImplTest {
    ...
    void map_mapsAllFields() {
        SystemBResponse response = new SystemBResponse(
                "xyz", "Jane", "Doe", "1990-01-01", true);
    ...
    void map_joinsFirstAndLastNameWithSpace() {
        SystemBResponse response = new SystemBResponse(
                "1", "Aliaksei", "Kozel", "2000-06-15", false);
    ...
    void map_activeFalse_whenNotActive() {
        SystemBResponse response = new SystemBResponse(
                "1", "John", "Smith", "1985-03-20", false);

        assertThat(mapper.map(response).active()).isFalse();
    }

    @Test
    void map_birthDateNotReflectedInResult() {
        SystemBResponse response = new SystemBResponse(
                "1", "John", "Smith", "1985-03-20", true);

        InsuranceData result = mapper.map(response);

        // InsuranceData has no birthDate field — verify model stays clean
