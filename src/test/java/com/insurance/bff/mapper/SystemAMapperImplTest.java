...
/**
 * Unit tests for {@link SystemAMapperImpl}.
 * Validates description parsing and field mapping in isolation.
 */
class SystemAMapperImplTest {
    ...
    void map_mapsIdAndParsesNameAndActiveStatus() {
        SystemAResponse response = new SystemAResponse(
                "abc", "Patient: Jane Doe, status: active");
    ...
    void map_activeFalse_whenStatusInactive() {
        SystemAResponse response = new SystemAResponse(
                "1", "Patient: John Smith, status: inactive");
    ...
    void map_nameWithComma_parsedCorrectly() {
        // lastIndexOf ensures only the final ", status: " separator is used
        SystemAResponse response = new SystemAResponse(
                "1", "Patient: Doe, Jane, status: active");
